...
package example.detailed.dialog;
...
public class UrlRequestDialog extends JDialog {
  ...
  public UrlRequestDialog(Frame owner, String title) {
    ...
    addRequestFlag(flagsPanel,
        "Skip cache",
        CefUrlRequestFlags.UR_FLAG_SKIP_CACHE,
    ...
    addRequestFlag(flagsPanel, "No download data",
        CefUrlRequestFlags.UR_FLAG_NO_DOWNLOAD_DATA,
        "If set the CefURLRequestClient.onDownloadData method will not be called", false);
    addRequestFlag(flagsPanel, "No retry on 5xx",
        CefUrlRequestFlags.UR_FLAG_NO_RETRY_ON_5XX,
        "If set 5XX redirect errors will be propagated to the observer instead of automatically re-tried.", false);
    ...
    headerTblModel.addEntry("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8");
    ...
    sendButton.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        UrlRequestDialogReply handleRequest = new UrlRequestDialogReply(owner_, getTitle() + " - Result");
  ...
  private void addRequestMode(JPanel panel, ButtonGroup buttonGrp, String requestMode, boolean selected) {
  ...
  private void addRequestFlag(JPanel panel, String flag, int value, String tooltip, boolean selected) {
